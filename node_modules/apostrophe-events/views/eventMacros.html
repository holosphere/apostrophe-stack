    ...
    <div class="apos-event-heading">
      <h4>
        <a href="{{ pageUrl }}/{{ item.slug }}">{{ item.title | e }}</a>      {% if options.edit and item._edit %}
          <a class="apos-control apos-ui apos-button apos-edit-snippet-button" data-edit-{{ item.type | css }} data-slug="{{ item.slug }}" href="#"><i class="fa fa-pencil"></i> Edit</a>
        {% endif %}
      </h4>
